...
//   =====================================
  ...
  test('Add new user', async ({ page }) => {
    ...
    if (await emailField.isVisible({ timeout: 2000 }).catch(() => false)) {
      await fillFieldWithDelay(emailField, userDataAdd.email);
    }
...
//   =====================================
  test('Edit user', async ({ page }) => {
    ...
    const userRows = page.locator('table tbody tr, [role="row"], .user-item, div[class*="user"]');
    ...
    const editIcon = userAtIndex0.locator('button')
    ...
    if (await roleDropdown.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (await districtField.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (!(await districtField.isDisabled())) {
          ...
          await fillFieldWithDelay(districtField, userDataEdit.address.district);
        }
      }
...
//   =====================================
  ...
  test('Delete user', async ({ page }) => {
    ...
    if (await userToDelete.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const deleteIconInRow = userToDelete.locator('button').filter({ 
